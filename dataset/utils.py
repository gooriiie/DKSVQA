...
def vqa_eval(vqa, result_file, test_ques_path, logger=None, n=2, dataset='vqa'):
    ...
    # evaluate results
    if dataset != 'aokvqa':
        vqaEval.evaluate(logger=logger)
    else:
    ...
    if logger is None:
        ...
        logger.info("Per Question Type Accuracy is the following:")
        for quesType in vqaEval.accuracy['perQuestionType']:
            logger.info("%s : %.02f" % (quesType, vqaEval.accuracy['perQuestionType'][quesType]))
        logger.info("\n")

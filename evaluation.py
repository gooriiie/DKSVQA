...
def create_task_prompt(syn_question_queid,syn_ans_queid,syn_question_queid_next,config):
    ...
    for idx in range(20):
        ...
        if config['dataset'] in ['vqa', 'vqasubset', 'vqatest'] and config['question_type'] != 'rule' and config[
                'num_question_per_img'] > 0 and idx < 1:  ## yes and no questions for vqav2p
...
def evaluation(model, test_data,caption_dict,syn_question_dict,syn_answer_dict,ans_to_cap_dicts,tokenizer, device, config, logger, writer, split='test',resume=False,result_dir = 'output/VQA_caption/result', result_filename = 'result'):
    ...
    for n, per_test_data in enumerate(metric_logger.log_every(test_data, print_freq, header)):
        ...
        Prompt = "Answer the question based on the given context and knowledge. Answer in one or two words.\n"
        ...
        # pagerank knowledge ===================================================
        pagerank_kb = load_json(os.path.join(config['llm_selected_kb'], str(question_id) + '.json'))    
        kb1 = pagerank_kb[0:2]
        # ======================================================================
        
        kb2 = load_json(os.path.join(config['sim_selected_kb'], str(question_id) + '.json'))
        kb = list(set(kb1+ kb2))
        

        Knowledge_Prompt = ' '.join(kb)

        DKSVQAPrompt = "Knowledge:" + Knowledge_Prompt + "\n" + "Contexts:" + Context_Prompt + "\n" + Task_Prompt + "Question:" + question + "\nAnswer:"
        ...
        DKSVQAPrompt_input = tokenizer(DKSVQAPrompt, padding='longest', truncation=True, return_tensors="pt").to(
            device)
        opt_input.input_ids = torch.cat((opt_input.input_ids, DKSVQAPrompt_input.input_ids), dim=-1)
        opt_input.attention_mask = torch.cat((opt_input.attention_mask, DKSVQAPrompt_input.attention_mask), dim=-1)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            try:
                ...
                output_scores = torch.reshape(output_scores, (-1, int(dim_1 / len(outputs.scores)))).float()
...
def update(params, args):
    ...
    params['llm_selected_kb'] = args.llm_selected_kb
    params['sim_selected_kb'] = args.sim_selected_kb
...
if __name__ == '__main__':
    ...
    parser.add_argument('--llm_selected_kb')
    parser.add_argument('--sim_selected_kb')

    parser.add_argument('--ans_dict_file')
